...
app.get('/api/mysql/:table', async (req, res) => {
    ...
    const allowed = ['aset', 'kategori_aset', 'lokasi', 'maintenance', 'mutasi_aset', 'peminjaman', 'penghapusan', 'penyusutan', 'supplier', 'user', 'log_activity'];
...
app.post('/api/mysql/login', async (req, res) => {
    try {
        ...
        const [rows] = await pool.query('SELECT * FROM user WHERE username = ? AND status = "AKTIF"', [username]);
        if (rows.length === 0) {
        ...
        res.json({ success: true, message: 'Login berhasil', user: { user_id: user.user_id, username: user.username, nama_lengkap: user.nama_lengkap, role: user.role } });
    } catch (err) {
